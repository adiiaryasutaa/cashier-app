<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    <div class="flex items-start justify-between px-6 py-4 bg-white">
        ...
        <x-button type="button">{{ __('Add User') }}</x-button>
    </div>
        ...
        @foreach($users as $user)
            <tr class="bg-white border-b hover:bg-gray-50">
                <th scope="row" class="flex flex-col items-start px-6 py-4 text-gray-900 whitespace-nowrap">
                    <div class="text-base font-semibold text-start">{{ $user->name }}</div>
                    <div class="font-normal text-gray-500 text-start">{{ $user->username }}</div>
                </th>
                <td class="px-6 py-4">
                    {{ $user->role->label() }}
                </td>
                ...
                <td class="px-6 py-4">
                    <a href="{{ route('users.show', ['user' => $user]) }}" class="font-medium text-blue-600 hover:underline">{{ __('Detail') }}</a>
                </td>
            </tr>
        @endforeach
        </tbody>
