    ...
    <div class="flex items-start justify-between px-6 py-4 bg-white">
        <div class="relative mt-1">
            ...
            <input wire:model.live.debounce="search" type="text"
                   ...
                   placeholder="{{ __('Search for transactions') }}">
        </div>
        ...
        @foreach($transactions as $transaction)
            <tr class="bg-white border-b hover:bg-gray-50">
                <th scope="row" class="flex flex-col items-start px-6 py-4 text-gray-900 whitespace-nowrap">
                    <div class="text-base font-semibold text-start">{{ $transaction->code }}</div>
                </th>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4 text-end">
                    {{ Number::currency($transaction->payment->price_price ?? 0, 'IDR') }}
                </td>
                <td class="px-6 py-4">
                    <a href="{{ route('transactions.show', ['transaction' => $transaction]) }}" class="font-medium text-blue-600 hover:underline">{{ __('Detail') }}</a>
                </td>
            </tr>
        @endforeach
        </tbody>
